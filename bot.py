...
bot = nanogram.api.Bot(bottoken, verifyToken=False)
#  pll = nanogram.api.PollingUpdater(bot)
#  pll.clearUpdates()
admins = [1710409403, 999168577]
cfgs = json.loads(open("config.json").read())
limit = cfgs["limit"]

# Aqui handeeeeeeeeeeeeeeeeel
lm = 5000000
...
cache: dict[int, int] = {999168577: 40000}
            ...
            for c in range(0, len(self.downloaded_list)):
                while True:
                    try:
                        ...
                        if self.cht in cache:
                            cache[self.cht] = cache[self.cht] - int(os.path.getsize(self.downloaded_list[c])) / 1024 / 1024
                        os.remove(self.downloaded_list[c])
